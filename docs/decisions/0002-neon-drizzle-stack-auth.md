...
2. Auth — replaced by Neon Auth (Stack).
...
**Neon Postgres + Drizzle ORM + Neon Auth (Stack) + Vercel Blob**, with authorization
...
- Identity lives in our own Postgres (`neon_auth.users_sync`), so profile joins are SQL,
  not network calls.
