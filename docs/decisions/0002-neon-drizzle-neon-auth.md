...
- A single authorization bug reaches production, or a review finds a query that skipped
  ...
  Neon RLS with JWT-scoped policies keyed off the Neon Auth session — a real migration,
  but a known one.
...
> **Read the correction at the bottom of this ADR before acting on the auth row.** Neon
> Auth is a hosted Better Auth instance, not Stack Auth.
...
| Supabase Auth         | Neon Auth (hosted Better Auth), via `lib/auth` — see the correction below                          |
